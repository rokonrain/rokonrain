...
@section('sidebar')
    ...
    <h2 style="color: white; margin-left:auto">BLPA Passenger Terminal Porter List </h2>
@endsection
...
@section('content')
    ...
    <div class="contenitore">
                        ...
                        <div class="row">
                            <div class="col-lg-12" id="head">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <a href="{{ route('posts.index') }}" class=" mt-1">
                                                        <span class="input-group-btn mr-1 mt-1">
                                                            <button class="btn btn-info" type="submit" title="Search Porter's Name">
                                ...
                                <div class="pull-right mr-5 mt-1">
                                    <a class="btn btn-success" href="{{ route('posts.create') }}"> <i class="fas fa-plus"> </i> Create
                                        new Porter</a>
                                </div>
                            </div>
                        </div>
            ...
            <table class="table table-striped table-light table-hover table-responsive" id="head"
                ...
                <tr>
                    @foreach ($posts as $post)
                        ...
                        <td>{{ $post->punion }}</td>
                        <td>{{ $post->pthana }}</td>
                        <td>{{ $post->pdistrict }}</td>
                        <td>{{ $post->pdivision }}</td>
                        <td>{{ $post->nid }}</td>
                        <td>{{ $post->mobile }}</td>
        ...
        <script type="text/javascript">
            ...
            $("#first").attr("class", "nav-link active");
        </script>
    </div>
@endsection
